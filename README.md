...
<!-- StoryBreeze는 ‘맞춤형 교재 생성 AI 서비스’라는 명확한 방향성과 체계적인 BM 모델을 갖춘 서비스로서, 개인화된 학습 환경 조성과 디지털 콘텐츠 플랫폼을 통한 시장 경쟁력을 강화할 수 있을 것으로 기대합니다. -->
...
|:-:|:-:|
|사이트 내 결제 시스템을 통해 유료로 리프를 구매<br>⬇️<br>리프에는 두 종류가 존재<br>⬇️<br>일반 리프: 다량으로 구매 가능 (예: 100개 단위 등) ＆ 골든 리프: 일반 리프 100개에 해당하는 가치로, 대량 패키지나 고가 콘텐츠 생산 시 사용|도서 1권 생성 시 일정량의 리프가 소모<br>⬇️<br>책의 분량, 퀄리티, 콘텐츠 난이도 등에 따라 리프 소모량이 달라짐<br>⬇️<br>사용자 입장에서는 책 1권당 기존 서적보다 저렴한 비용으로 원하는 책을 얻을 수 있음|

### `추가 수익 모델`<br>
|① 프리미엄 기능 판매|② 광고 및 스폰서십|
|:-:|:-:|
|AI에게 추가 검색 확장이나, 특정 전문 지식(특허 정보 등)에 대한 접근권을 프리미엄 결제로 제공|특정 분야(예: 요리, 다이어트, IT 등)에 광고주 스폰서십 책자를 생성할 수 있도록 협업|
|특수 카테고리(전문 논문, 희귀 자료) 생성 시 추가 리프 필요|관련 브랜드가 책의 표지나 내용 중간에 광고 콘텐츠로 노출 가능|

## Flow-Chart
### 1. 사이트 접속
|`랜딩 페이지`|`회원가입/로그인 모달창`|`로그인 후 접근 가능 메뉴`|
|:-:|:-:|:-:|
|StoryBreeze 서비스 소개(핵심 기능, 장점, 사용 방법 등)|버튼 클릭 시 모달창 팝업|커뮤니티|
|회원가입/로그인을 유도하는 버튼 노출|이용약관·개인정보처리방침 확인 후 동의 절차 진행|AI 도서 생성|
||Kakao 간편 로그인(Kakao API 활용) 또는 이메일+비밀번호 방식으로 회원가입/로그인 가능|마이페이지|
||회원가입 시 필요한 정보(아이디, 비밀번호, 이름, 생년월일, 연락처, 주소, 이메일)|리프 충전|

### 2. AI 도서 생성
<img src="https://github.com/user-attachments/assets/55192e25-d873-401c-bbcd-37efa70c66c4" width="650" alt="ai_create_book">

|`AI 도서 생성 메뉴 진입`|`카테고리 선택`|`도서 생성`|`책 저장 및 열람`|
|:-:|:-:|:-:|:-:|
(메인화면 레이아웃 예시)<br>좌측 메뉴바: 카테고리 선택, 내 서재 이동 등 주요 버튼<br>상단 그리기 바: (추후 편집 기능, 디자인 기능 등 확장 가능)<br>중앙 영역: 책 출력 미리보기<br>우측 영역: 카테고리 세분화 및 선택 기능|우측 카테고리 공간에서 초기(1차) 카테고리 선택|모든 카테고리 선택이 끝나면 “AI 생성하기” 버튼을 클릭|생성된 책은 자동으로 내 서재에 저장됨|
||“다음으로” 버튼 클릭 시 세분화된 카테고리(2차, 3차, …)를 단계적으로 선택 가능|선택된 카테고리를 기반으로 GPT API가 표지, 목차, 내용 구성|사용자는 좌측 메뉴바에서 ‘내 서재’ 메뉴에 접근하여 생성된 도서를 확인|
||깊은 Depth의 카테고리는 GPT API를 통해 실시간으로 불러와 사용자에게 제안|(생성 과정)<br>목차 기반으로 한 페이지씩 AI가 생성<br>⬇️<br>페이지 로딩 시 글자가 한글자씩 페이드인 효과 적용<br>⬇️<br>다음 페이지 생성 전 로딩 화면(로딩 서클) 표시<br>⬇️<br>페이지 넘김 시 북 플립(Book Flip) 애니메이션 적용|책을 클릭하면 이전에 읽었던 페이지부터 이어보기 가능|
||“이전 카테고리” 버튼을 통해 이전 단계로 돌아갈 수 있음||책은 페이지를 넘길 때마다 새로운 내용을 생성하므로, 즉시 전부 다 생성하기보다 필요 시 생성하는 효율적 구조|
